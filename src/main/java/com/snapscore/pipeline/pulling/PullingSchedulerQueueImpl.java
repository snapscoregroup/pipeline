...
//                    jsonLog.debug("Running regular trigger of pullNext()");
    ...
    private synchronized void enqueueRequest(FeedRequest feedRequest, Consumer<PullResult> pullResultConsumer, Consumer<PullError> pullErrorConsumer) {
        if (shouldMakeRequest(feedRequest)) {
            ...
            logger.decorateSetup(mdc -> mdc.anyId(feedRequest.getUuid()).exec("ignoring_duplicate_request")).info("FeedRequest already enqueued for pulling - ignoring: {}; ", feedRequest.toStringBasicInfo());
        }
    }
    ...
    private boolean isAwaitingForTooLong(FeedRequest feedRequest) {
        ...
        if (trackedRequest.isPresent()) {
            ...
            if (trackedDuration.getSeconds() > 10 * 60_000) { // 10 mins
                logger.decorateSetup(mdc -> mdc.anyId(feedRequest.getUuid()).exec("request_tracked_too_long")).warn(">>> !!! Request is tracked for too long! It is possible that some pulling results are not getting emitted by the http client callback implementation and it is causing requests being incorrectly ignored !!! <<< {}", feedRequest);
                return true;
            }
        }
        return false;
    ...
    private void logRequestError(FeedRequest request, Throwable error) {
        logger.decorateSetup(mdc -> mdc.anyId(request.getUuid()).exec("request_error")).warn("Error for request: {}", request.toStringBasicInfo(), error);
    }

    private void logRetry(FeedRequest request) {
        logger.decorateSetup(mdc -> mdc.anyId(request.getUuid()).exec("request_retry")).info("Going to retry request after previous failure {}", request.toStringBasicInfo());
    }

    private void logDelayedRetry(FeedRequest request) {
        logger.decorateSetup(mdc -> mdc.anyId(request.getUuid()).exec("request_retry_delay")).info("Cannot retry request yet - due to rqs per sec. limit {}", request.toStringBasicInfo());
    }
    ...
    private void logDroppingRetrying(FeedRequest request, Throwable error) {
        logger.decorateSetup(mdc -> mdc.anyId(request.getUuid()).exec("dropped_failed_request")).warn("Dropping request retry {} after error: ", request.toStringBasicInfo(), error);
    }
