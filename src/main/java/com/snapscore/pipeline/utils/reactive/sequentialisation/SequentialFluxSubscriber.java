...
public class SequentialFluxSubscriber<I, R> {
    ...
    private Consumer<? super R> getSubscribeConsumerWrapped(long itemEnqueuedTs) {
        Consumer<? super R> subscribeConsumerWrapper = result -> {
            ...
            loggingInfo.decorate(logger).decorateSetup(props -> props.analyticsId("input_processing_time").exec(Long.toString(processingTimeMillis))).info("Input took {} ms to process: {}", processingTimeMillis, loggingInfo.getMessage());
        };
